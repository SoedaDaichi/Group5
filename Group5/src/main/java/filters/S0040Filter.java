...
public class S0040Filter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		// 商品検索系のsession破棄
		boolean isTargetPage = uri.matches(".*/S004[0-4]\\.(html|jsp)$");
		String[] SessionKeys = { "asForm", "accountList", "accountId", "accounts", "accountsData" };

		if (session != null && !isTargetPage) {
			for (String accountsSessionKey : SessionKeys) {
				if (session.getAttribute(accountsSessionKey) != null) {
					session.removeAttribute(accountsSessionKey);
					System.out.println("アカウント検索系: " + accountsSessionKey + "を削除。");
				}
			}
		}
